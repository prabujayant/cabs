...
const LoginPage: React.FC = () => {
  ...
  const [age, setAge] = useState<number>(25); // Default age value
    ...
    if (name && username && password && age) {
      try {
        ...
        const docRef = await addDoc(collection(db, "users"), {
          name,
          username,
          password, // Storing password (consider hashing it before storing it in a real app)
        ...
        router.push({
          pathname: "/actdetails", 
          params: { userName: name, userAge: age } // Passing data to the next page
        });
      } catch (e) {
